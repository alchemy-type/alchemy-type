...
class PracticeTextEntry extends Component {
  render() {
    ...
    let currentChar = handleCurrentChar(
      passageParent,
      passageParent.children[0],
      userInputLength
    );
    ...
    userInput.addEventListener("input", event => {
      ...
      if (enterFlag && matchFlag) {
        ...
        if (userInputLength < passageParent.children.length) {
          let viewportOffset = passageParent.children[
            userInputLength
          ...
          if (
            viewportOffset.bottom + viewportOffset.height + 100 >
            viewportHeight
          ) {
            setTimeout(() => {
